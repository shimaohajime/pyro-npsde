...
def fit_model(sess,model,t,Y,Nw=10,num_iter=500,print_every=10,eta=5e-3,dec_step=20,dec_rate=0.99,plot_=True, cost_function = "noise"):
    ...
    if model.name is 'npsde':
        with tf.name_scope("cost"):
            Xs = model.forward(x0,t,Nw=Nw)

            if cost_function == "noise":
                ll = 0
                for i in range(len(Y)):
                ...
                cost = -(ll + sde_prior)
            elif cost_function == "KD" :

                # TODO: Implement KD-ranking cost function 
                ll = 0

                # Get base points
                for i in range(len(Y)):
                    base = Xs[i] # L x Nw x D 
                    actual  = Y[i] # L x D 
                    ...
                    for j in range(base.shape[0]):
                        kde = KernelDensity(bandwidth=1.0, kernel='gaussian').fit(base[i,:,:]) # TODO: Make bandwidth a learnable parameter
                        actual_point = actual[j,:]
                        # log_prob_base = kde.score_samples(base[i,:,:])
                        log_prob_actual = kde.score(actual_point)
                        pointwise += [(None, log_prob_actual)]

                    # Aggregate KD rankings 
                    ll += sum([x[1] for x in pointwise])
